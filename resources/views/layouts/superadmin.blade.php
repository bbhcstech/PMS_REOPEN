    ...
    <div x-data="{ sidebarOpen: false }" class="min-h-screen lg:flex">
        ...
        <aside class="pms-super-sidebar fixed inset-y-0 left-0 z-40 flex w-72 -translate-x-full flex-col bg-[#152047] text-white shadow-2xl transition-transform duration-200 lg:static lg:translate-x-0"
            ...
            <nav class="flex-1 space-y-3 overflow-y-auto px-4 py-7 text-base font-semibold">
                @php
                    $items = [
                        ['Companies', Route::has('super-admin.companies.index') ? route('super-admin.companies.index') : '#', request()->routeIs('super-admin.companies.*'), 'M3 21h18M5 21V5a2 2 0 012-2h7a2 2 0 012 2v16M9 7h1m-1 4h1m4-4h1m-1 4h1M9 21v-4h4v4'],
                    ];
                @endphp
            ...
            <form method="POST" action="{{ Route::has('super-admin.logout') ? route('super-admin.logout') : route('logout') }}" class="border-t border-white/10 p-4">
                @csrf
                ...
                <div class="flex h-16 items-center justify-between px-4 sm:px-6 lg:px-8">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="hidden text-xs font-medium text-slate-500 sm:block">Control tenant companies, databases, and platform settings.</p>
                        </div>
                    </div>
                    <div class="flex items-center gap-3">
                        ...
                        <div class="hidden rounded-full bg-slate-100 px-4 py-2 text-sm font-semibold text-slate-700 md:block">{{ auth('super_admin')->user()?->email ?? auth()->user()?->email }}</div>
