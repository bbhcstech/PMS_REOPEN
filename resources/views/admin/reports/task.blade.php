...
<div class="container-fluid px-4 py-4">
    ...
    <div class="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-4">
        <div>
            <h3 class="fw-bold mb-0 text-dark"><i class="bx bx-task text-primary me-2"></i> Task Report</h3>
    ...
    <div class="row g-3 mb-4">
        <div class="col-lg-3 col-md-6">
            <div class="card border-0 shadow-sm rounded-4 text-center p-3 bg-white h-100 border-start border-primary border-4">
                <small class="text-muted text-uppercase fw-bold mb-1">Total Tasks</small>
                <h3 class="fw-extrabold text-primary mb-0">{{ $stats['total'] }}</h3>
            </div>
        </div>

        <div class="col-lg-3 col-md-6">
            <div class="card border-0 shadow-sm rounded-4 text-center p-3 bg-white h-100 border-start border-success border-4">
                <small class="text-muted text-uppercase fw-bold mb-1">Completed Tasks</small>
                <h3 class="fw-extrabold text-success mb-0">{{ $stats['completed'] }}</h3>
            </div>
        </div>

        <div class="col-lg-3 col-md-6">
            <div class="card border-0 shadow-sm rounded-4 text-center p-3 bg-white h-100 border-start border-info border-4">
                <small class="text-muted text-uppercase fw-bold mb-1">In Progress</small>
                <h3 class="fw-extrabold text-info mb-0">{{ $stats['in_progress'] }}</h3>
            </div>
        </div>

        <div class="col-lg-3 col-md-6">
            <div class="card border-0 shadow-sm rounded-4 text-center p-3 bg-white h-100 border-start border-warning border-4">
                <small class="text-muted text-uppercase fw-bold mb-1">Pending / Open</small>
                <h3 class="fw-extrabold text-warning-emphasis mb-0">{{ $stats['pending'] }}</h3>
            </div>
        </div>
    </div>
            ...
            <form method="GET" action="{{ route('reports.task') }}">
                <div class="row g-3 align-items-end">
                    ...
                    <div class="col-lg-3 col-md-3">
                        <label class="form-label fw-semibold text-dark mb-1">Filter by Status</label>
                    ...
                    <div class="col-lg-5 col-md-4 d-flex align-items-center gap-2">
                        ...
                        <a href="{{ route('reports.task') }}" class="btn btn-outline-secondary rounded-pill px-4 flex-grow-1 text-nowrap">
                    ...
                    <tbody>
                        @foreach($tasks as $key => $task)
                            <tr>
                                ...
                                <td><div class="fw-bold text-dark">{{ $task->title }}</div></td>
                                <td>{{ $task->project->name ?? 'General' }}</td>
                                <td><span class="badge bg-light text-dark border">{{ $task->category->category_name ?? 'Task' }}</span></td>
                                <td>
                                    @php
                                        $pBadge = ['low'=>'secondary','medium'=>'info','high'=>'warning','critical'=>'danger'][strtolower((string)$task->priority)] ?? 'light';
                                    @endphp
