...
<div class="container-fluid px-4 py-4">
    <div class="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-4">
        <div>
            <h3 class="fw-bold mb-0 text-dark"><i class="bx bx-cart text-primary me-2"></i> Sales Report</h3>
    ...
    <div class="row g-3 mb-4">
        <div class="col-lg-6 col-md-6">
            <div class="card border-0 shadow-sm rounded-4 text-center p-3 bg-white h-100 border-start border-primary border-4">
                <small class="text-muted text-uppercase fw-bold mb-1">Total Sales Transactions</small>
                <h3 class="fw-extrabold text-primary mb-0">{{ $sales->count() }}</h3>
            </div>
        </div>

        <div class="col-lg-6 col-md-6">
            <div class="card border-0 shadow-sm rounded-4 text-center p-3 bg-white h-100 border-start border-success border-4">
                <small class="text-muted text-uppercase fw-bold mb-1">Total Sales Revenue</small>
                <h3 class="fw-extrabold text-success mb-0">${{ number_format($totalSales, 2) }}</h3>
            </div>
        </div>
    </div>
                    ...
                    <tbody>
                        @foreach($sales as $key => $sale)
                            <tr>
                                ...
                                <td><div class="fw-bold text-dark">Invoice #{{ $sale->invoice_id ?? $sale->id }}</div></td>
                                <td>{{ $sale->project->name ?? 'General Sales' }}</td>
                                <td><span class="badge bg-success-subtle text-success fw-bold fs-6 px-3 py-1">${{ number_format($sale->amount, 2) }}</span></td>
                                <td>{{ $sale->created_at ? \Carbon\Carbon::parse($sale->created_at)->format('d M Y') : '—' }}</td>
                                <td><span class="badge bg-success rounded-pill px-3 py-1">Completed</span></td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
