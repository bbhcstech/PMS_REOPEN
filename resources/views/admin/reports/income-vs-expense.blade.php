...
<div class="container-fluid px-4 py-4">
    <div class="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-4">
        <div>
            <h3 class="fw-bold mb-0 text-dark"><i class="bx bx-trending-up text-primary me-2"></i> Income Vs Expense Report</h3>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0">
                    <li class="breadcrumb-item"><a href="{{ route('dashboard') }}" class="text-decoration-none">Dashboard</a></li>
                    <li class="breadcrumb-item active" aria-current="page">Income Vs Expense</li>
                </ol>
            </nav>
        </div>
    </div>
    ...
    <div class="row g-3 mb-4">
        <div class="col-lg-4 col-md-6">
            <div class="card border-0 shadow-sm rounded-4 text-center p-3 bg-white h-100 border-start border-success border-4">
                <small class="text-muted text-uppercase fw-bold mb-1">Total System Income</small>
                <h3 class="fw-extrabold text-success mb-0">${{ number_format($totalIncome, 2) }}</h3>
            </div>
        </div>

        <div class="col-lg-4 col-md-6">
            <div class="card border-0 shadow-sm rounded-4 text-center p-3 bg-white h-100 border-start border-danger border-4">
                <small class="text-muted text-uppercase fw-bold mb-1">Total System Expense</small>
                <h3 class="fw-extrabold text-danger mb-0">${{ number_format($totalExpense, 2) }}</h3>
            </div>
        </div>

        <div class="col-lg-4 col-md-6">
            <div class="card border-0 shadow-sm rounded-4 text-center p-3 bg-white h-100 border-start border-primary border-4">
                <small class="text-muted text-uppercase fw-bold mb-1">Net Balance Ratio</small>
                <h3 class="fw-extrabold text-primary mb-0">${{ number_format($totalIncome - $totalExpense, 2) }}</h3>
            </div>
        </div>
    </div>
    ...
    <div class="card border-0 shadow-sm rounded-4">
        <div class="card-body p-4">
            <div class="table-responsive">
                <table id="incomeVsExpenseTable" class="table table-hover align-middle w-100">
                    ...
                    <tbody>
                        @foreach($payments as $key => $p)
                            <tr>
                                ...
                                <td><div class="fw-bold text-dark">{{ date('F Y', mktime(0, 0, 0, $p->month, 1, $p->year)) }}</div></td>
                                <td><span class="badge bg-success-subtle text-success fw-bold fs-6 px-3 py-1">${{ number_format($p->total_income, 2) }}</span></td>
                                <td><span class="badge bg-success rounded-pill px-3 py-1">Audited</span></td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
...
<script>
    $(document).ready(function () {
        $('#incomeVsExpenseTable').DataTable({
