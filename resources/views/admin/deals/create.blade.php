...
<div class="container-fluid py-3">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h4 class="fw-bold mb-1 d-flex align-items-center gap-2" style="color: #0f172a;">
                <span class="fs-4">➕</span> Create New Deal
            </h4>
            <p class="text-muted small mb-0">Add a new sales opportunity into your pipeline</p>
        </div>
    ...
    {{-- FORM CARD --}}
    <form action="{{ route('admin.deals.store') }}" method="POST" id="dealCreateForm">
        @csrf

        <div class="card form-card-modern mb-4">
            <div class="card-header form-card-header-modern">
                <h6 class="fw-bold mb-0 text-dark d-flex align-items-center gap-2"><span class="fs-5">🤝</span> Deal Opportunity Details</h6>
            </div>
            <div class="card-body p-4">
                <div class="row g-3">
                    <div class="col-md-6">
                        <label class="form-label-modern">Deal Name <span class="text-danger">*</span></label>
                        <input type="text" name="deal_name" class="form-control form-control-modern @error('deal_name') is-invalid @enderror" value="{{ old('deal_name') }}" required placeholder="e.g. Enterprise License Expansion">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="lead_id" id="leadSelect" class="form-select form-select-modern">
                            ...
                            @foreach($leads as $ld)
                                <option value="{{ $ld->id }}"
                                    data-name="{{ $ld->contact_name }}"
                                    data-company="{{ $ld->company_name }}"
                                    data-contact="{{ $ld->email . ($ld->phone ? ' | ' . $ld->phone : '') }}"
                                    {{ old('lead_id', $selectedLead->id ?? '') == $ld->id ? 'selected' : '' }}>
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="text" name="lead_name" id="inputLeadName" class="form-control form-control-modern @error('lead_name') is-invalid @enderror" value="{{ old('lead_name', $selectedLead->contact_name ?? '') }}" required placeholder="e.g. John Doe">
                        @error('lead_name')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>

                    <div class="col-md-4">
                        <label class="form-label-modern">Company Name</label>
                        <input type="text" name="company_name" id="inputCompanyName" class="form-control form-control-modern" value="{{ old('company_name', $selectedLead->company_name ?? '') }}" placeholder="e.g. Acme Inc">
                    </div>

                    <div class="col-md-4">
                        <label class="form-label-modern">Contact Details <span class="text-danger">*</span></label>
                        <input type="text" name="contact_details" id="inputContactDetails" class="form-control form-control-modern @error('contact_details') is-invalid @enderror" value="{{ old('contact_details', isset($selectedLead) ? $selectedLead->email . ($selectedLead->phone ? ' | ' . $selectedLead->phone : '') : '') }}" required placeholder="Email / Phone">
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="number" step="0.01" name="value" id="inputValue" class="form-control form-control-modern @error('value') is-invalid @enderror" value="{{ old('value', 0) }}" required placeholder="0">
                    ...
                    <div class="col-md-3">
                        ...
                        <select name="deal_stage_id" id="stageSelect" class="form-select form-select-modern" required>
                            @foreach($stages as $stg)
                                <option value="{{ $stg->id }}" data-prob="{{ $stg->default_probability }}" {{ old('deal_stage_id') == $stg->id ? 'selected' : '' }}>
                    ...
                    <div class="col-md-3">
                        ...
                        <input type="number" min="0" max="100" name="probability" id="inputProbability" class="form-control form-control-modern" value="{{ old('probability', 10) }}" placeholder="10">
                    </div>

                    <div class="col-md-4">
                        <label class="form-label-modern">Pipeline</label>
                        <select name="pipeline" class="form-select form-select-modern">
                            <option value="Sales Pipeline" {{ old('pipeline') == 'Sales Pipeline' ? 'selected' : '' }}>Sales Pipeline</option>
                            <option value="Marketing Pipeline" {{ old('pipeline') == 'Marketing Pipeline' ? 'selected' : '' }}>Marketing Pipeline</option>
                            <option value="Other Pipeline" {{ old('pipeline') == 'Other Pipeline' ? 'selected' : '' }}>Other Pipeline</option>
                        </select>
                    </div>
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="deal_category_id" class="form-select form-select-modern">
                            ...
                            @foreach($categories as $cat)
                                <option value="{{ $cat->id }}" {{ old('deal_category_id') == $cat->id ? 'selected' : '' }}>{{ $cat->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="deal_agent_id" class="form-select form-select-modern">
                            ...
                            @foreach($agents as $ag)
                                <option value="{{ $ag->id }}" {{ old('deal_agent_id', auth()->id()) == $ag->id ? 'selected' : '' }}>{{ $ag->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="date" name="close_date" class="form-control form-control-modern @error('close_date') is-invalid @enderror" value="{{ old('close_date', date('Y-m-d', strtotime('+30 days'))) }}" required>
                        @error('close_date')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>

                    <div class="col-md-4">
                        <label class="form-label-modern">Next Follow-up Date</label>
                        <input type="date" name="next_follow_up" class="form-control form-control-modern" value="{{ old('next_follow_up') }}" placeholder="dd-mm-yyyy">
                    </div>

                    <div class="col-md-12">
                        <label class="form-label-modern">Product / Service Description</label>
                        <input type="text" name="product" class="form-control form-control-modern" value="{{ old('product') }}" placeholder="e.g. ERP Software License 1-Year Subscription">
                    </div>

                    <div class="col-md-12">
                        <label class="form-label-modern">Notes / Description</label>
                        <textarea name="notes" class="form-control form-control-modern" rows="3" placeholder="Enter opportunity details, negotiation notes, or client preferences...">{{ old('notes') }}</textarea>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="d-flex justify-content-end gap-2 mb-5">
            ...
            <button type="submit" class="btn-submit-emerald"><i class="fas fa-check me-1"></i> Save Deal</button>
        </div>
    </form>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const inputLeadName = document.getElementById('inputLeadName');
    const inputCompanyName = document.getElementById('inputCompanyName');
    const inputContactDetails = document.getElementById('inputContactDetails');
    ...
    if (leadSelect) {
        leadSelect.addEventListener('change', function() {
            ...
            if (opt && opt.value) {
                if (inputLeadName) inputLeadName.value = opt.dataset.name || '';
                if (inputCompanyName) inputCompanyName.value = opt.dataset.company || '';
                if (inputContactDetails) inputContactDetails.value = opt.dataset.contact || '';
            }
        });
    }
