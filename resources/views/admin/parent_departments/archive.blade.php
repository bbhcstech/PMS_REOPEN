...
@section('title', 'Archived Parent Departments')
...
<style>
    ...
    /* Level Badges - All with WHITE text */
    .level-badge {
        display: inline-flex;
        align-items: center;
        gap: 5px;
    ...
    <div class="content-wrapper">
        ...
        <div class="archive-header">
            <div class="header-title">
                <h1>
                    <i class="fas fa-archive me-2"></i>Archived Parent Departments
                    <span class="badge-premium"><i class="fas fa-history me-1"></i>ARCHIVE</span>
                </h1>
                <p><i class="fas fa-info-circle me-1"></i>Restore archived parent department records whenever they need to return to the active list.</p>
            </div>
            <a href="{{ route('parent-departments.index') }}" class="back-button">
                <i class="fas fa-arrow-left"></i>Back to Active Parent Departments
            </a>
        </div>
        ...
        @php
            ...
            $subDepartmentCount = $archivedCollection->sum('departments_count');
            $employeeCount = $archivedCollection->sum('employees_count');
        ...
        <div class="stats-grid">
            ...
            <div class="stat-card">
                <div class="stat-icon levels"><i class="fas fa-sitemap"></i></div>
                <div class="stat-info">
                    <h6>Sub Departments</h6>
                    <h3>{{ $subDepartmentCount }}</h3>
                </div>
            </div>
            <div class="stat-card">
                <div class="stat-icon top"><i class="fas fa-users"></i></div>
                <div class="stat-info">
                    <h6>Employees</h6>
                    <h3>{{ $employeeCount }}</h3>
                </div>
        ...
        <div class="table-card">
            <div class="card-header">
                <div>
                    <h5><i class="fas fa-history me-2"></i>Archived Parent Department Records</h5>
                    <p class="header-subtitle">Search archived parent departments and restore them to the active list.</p>
                </div>
                <span class="total-badge"><i class="fas fa-database"></i>Total: {{ $totalArchived }}</span>
            </div>

            <!-- Search Bar -->
            <form method="GET" action="{{ route('parent-departments.archive') }}" class="archive-search-bar">
                <div class="search-input">
                    <i class="fas fa-search"></i>
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search archived parent departments...">
                </div>
                ...
                @if(request()->hasAny(['search', 'per_page']))
                    <a href="{{ route('parent-departments.archive') }}" class="reset-button">
            ...
            <div class="table-responsive">
                @if($departments->isEmpty())
                    <div class="empty-state">
                        ...
                        <h5>No Archived Parent Departments Found</h5>
                        <p>There are no parent departments in the archive at the moment.</p>
                        <a href="{{ route('parent-departments.index') }}" class="back-button inline-back">
                            <i class="fas fa-arrow-left"></i>Back to Active Parent Departments
                        </a>
                    </div>
                @else
                    <table class="archive-table">
                        <thead>
                            <tr>
                                ...
                                <th><i class="fas fa-building"></i>Department Details</th>
                                <th><i class="fas fa-sitemap"></i>Sub Departments</th>
                                <th><i class="fas fa-users"></i>Employees</th>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <span class="code-badge">
                                            <i class="fas fa-qrcode"></i>{{ $department->dpt_code ?? 'N/A' }}
                                        </span>
                                    </td>
                                    <td>
                                        <div class="designation-cell">
                                            <div class="designation-avatar"><i class="fas fa-building"></i></div>
                                            <div>
                                                <div class="designation-name">{{ $department->dpt_name ?? '-' }}</div>
                                                <div class="designation-meta">ID: {{ str_pad($department->id, 3, '0', STR_PAD_LEFT) }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td>
                                        <span class="level-badge">
                                            <i class="fas fa-sitemap"></i>{{ $department->departments_count }}
                                        </span>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="action-group">
                                            <form action="{{ route('parent-departments.restore', $department->id) }}" method="POST" onsubmit="return confirm('Restore this parent department to the active list?');">
                                                @csrf
